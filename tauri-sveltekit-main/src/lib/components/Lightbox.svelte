<script lang="ts">
  import { files, lightboxIndex, closeLightbox, setLightboxIndex } from '$lib/store';
  ...
  const currentFile = $derived(
    files.value[lightboxIndex.value] || null
  );
  
  const thumbnailUrl = $derived(
    currentFile ? `/api/thumbnail/${currentFile.id}` : null
  );
  ...
  function handlePrevious() {
    if (lightboxIndex.value > 0) {
      setLightboxIndex(lightboxIndex.value - 1);
    }
  }
    ...
    <div class="lightbox-container">
      ...
      <div class="lightbox-image-container">
        ...
        {:else if currentFile.type === 'video'}
          <video src={currentFile.path} controls class="lightbox-video" />
        {:else}
      ...
      <div class="lightbox-info-bar">
        ...
        <div class="info-right">
          <span class="file-counter">
            {lightboxIndex.value + 1} / {files.value.length}
          </span>
          <button 
            class="btn-icon"
